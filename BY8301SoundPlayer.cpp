                                     ...
                                     bool aBusyState)
{
	...
	mpSerial = new SoftwareSerialTx(aTxPin);
...
BY8301SoundPlayer::~BY8301SoundPlayer()
{
	mpSerial->end();
...
void BY8301SoundPlayer::PlayFile(int aFile)
{
...
void BY8301SoundPlayer::PlayRepeat(int aFile)
{
	uint8_t lFileNumber = (uint8_t)aFile;
...
void BY8301SoundPlayer::SetVolume(int aVolume)
{
...
bool BY8301SoundPlayer::PlaySound(ESoundTypes aSoundType, uint16_t aIndex)
{
	...
	if(lSuccess)
	{
		...
		{
			//Send serial command to the BY8301 so that hum sound loops
			PlayRepeat(lSoundIndex);
		}
		else
		{
			//Send serial command to the BY8301 to play the sound
...
bool BY8301SoundPlayer::PlayRandomSound(ESoundTypes aSoundType)
{
...
const SoundFeatures& BY8301SoundPlayer::Features()
{
	return mpSoundMap->Features;
}


void BY8301SoundPlayer::SetFont(unsigned char aFontIdx)
{
	mFontIdx = aFontIdx;
}

#endif //BUILD_BY8301SoundPlayer
